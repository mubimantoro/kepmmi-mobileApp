...
class KepmmiRemoteMediator(
    private val apiService: ApiService,
    private val database: KepmmiDatabase
) : RemoteMediator<Int, KegiatanEntity>() {
    ...
    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, KegiatanEntity>
    ): MediatorResult {
        ...
        try {
            ...
            database.withTransaction {
                ...
                database.kepmmiDao().insertKegiatan(result)
            }
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, KegiatanEntity>): RemoteKeys? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let { data ->
            database.remoteKeysDao().getRemoteKeysId(data.id.toString())
        }
    }

    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, KegiatanEntity>): RemoteKeys? {
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()?.let { data ->
            database.remoteKeysDao().getRemoteKeysId(data.id.toString())
        }
    }

    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, KegiatanEntity>): RemoteKeys? {
